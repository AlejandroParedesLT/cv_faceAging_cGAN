...
parser.add_argument('--max_epoches', type=int, help='Number of epoches to run', default=5)
...
parser.add_argument('--age_classifier_path', type=str, help='directory of age classification model', default='./checkpoint/pretrain_alexnet/saved_parameters/epoch_4_iter_0.pth')
        ...
        for idx, (source_img_227,source_img_128,true_label_img,\
            ...
            if idx % args.val_interval == 0:
                ...
                for val_idx, (source_img_128, true_label_128) in enumerate(tqdm(test_loader)):
                    # Ensure the image tensor is valid after Reverse_zero_center transformation
                    img_tensor = Reverse_zero_center()(source_img_128)

                    if isinstance(img_tensor, torch.Tensor):  # Check if it's a tensor
                        # Ensure the tensor is on CPU and within the valid shape range for image saving
                        save_image(img_tensor.cpu(), os.path.join(save_dir, "batch_%d_source.jpg" % (val_idx)))
                    else:
                        logger.error("Invalid image tensor generated by Reverse_zero_center, skipping save.")
                    ...
                    for age in range(args.age_groups):
                        ...
                        # Ensure generated images are valid tensors
                        if isinstance(img, torch.Tensor):
                            save_image(Reverse_zero_center()(img).cpu(), os.path.join(save_dir, "batch_%d_age_group_%d.jpg" % (val_idx, age)))
                        else:
                            logger.error("Invalid image tensor generated for age group %d, skipping save." % age)

                logger.info('Validation images have been created!')
